...
function extractProps(): Map<string, ComponentMeta> {
  const discovered = [
    ...
    ...findComponentFiles(
      path.resolve(ROOT, 'ui/tucu-ui/src/datatable/components')
    ),
  ];
  // The first file a component is seen in wins (see the metadata.has check
  // below). Parse real source files before barrels so a component is credited
  // to the module that declares it — and so its own JSDoc is picked up —
  // rather than to whichever index.ts happens to re-export it first.
  const componentFiles = [
    ...discovered.filter((f) => !isBarrelFile(f)),
    ...discovered.filter(isBarrelFile),
  ];
...
function generateOutput(metadata: Map<string, ComponentMeta>): string {
  ...
  const lines: string[] = [
    // Descriptions are copied verbatim from source JSDoc, including third-party
    // prop docs that contain template-literal examples. Nothing here is
    // hand-written, so linting the output is noise.
