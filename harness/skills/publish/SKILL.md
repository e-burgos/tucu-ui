...
| --------------- | ---------------------------------------- | ------------------------------------------------------------ |
| `tucu-ui-v*.*.*` | `.github/workflows/publish-tucu-ui.yml` | build → verifica versión y artifacts → `npm publish` → GitHub Release (condicional) |
| `mcp-v*.*.*`     | `.github/workflows/publish-mcp.yml`     | build → verifica versión → `npm publish` → `flyctl deploy` → GitHub Release (condicional) |
...
Después de publicar, cada workflow decide si crear un **GitHub Release** —
nunca se crea a mano. Compara el tag recién pusheado contra el tag anterior
del mismo prefijo y crea el Release solo si el cambio es "significativo":

- bump de **major o minor** (cualquier bump de minor, no solo breaking), o
- un **patch** cuyos commits desde el tag anterior traen un marcador de
  breaking change (`feat!:`, `fix!:`, o un footer `BREAKING CHANGE:`).

Un patch normal sin marcador de breaking queda solo como tag — no genera
Release. Usa `--generate-notes` (notas automáticas desde los PRs/commits) y
`--latest=false` (los dos paquetes comparten repo, así que ninguno se marca
como "Latest release" para no pisarse entre sí).
